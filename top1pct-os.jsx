  ...
  return (
    <div style={{ background: 'var(--bg)' }}>
      ...
      <section className="hero reveal">
        <div className="hero-logo">◈ TOP 1% OPERATING SYSTEM</div>
        ...
        <div className="flex gap16">
          <button className="btn btn-primary" onClick={() => onNav(2)}>Get Your Score →</button>
          <button className="btn btn-ghost" style={{ border: '1px solid var(--glass-border)' }} onClick={() => onNav(1)}>Sign Up Free</button>
        </div>
      ...
      <footer className="footer">
        <div className="grid3">
          <div>
            <div className="fw700 mb16" style={{ letterSpacing: 2 }}>◈ TOP 1% OS</div>
            <div className="text-mute text-sm">Building the operating system for the world's most ambitious students.</div>
          </div>
          <div>
            <div className="text-accent fw700 mb16">Platform</div>
            <div className="text-dim text-sm mb8">Benchmarking</div>
            <div className="text-dim text-sm mb8">Roadmaps</div>
            <div className="text-dim text-sm">Shadow Network</div>
          </div>
          <div>
            <div className="text-accent fw700 mb16">Connect</div>
            <div className="text-dim text-sm mb8">Twitter / X</div>
            <div className="text-dim text-sm mb8">Discord</div>
            <div className="text-dim text-sm">Contact Support</div>
          </div>
        </div>
        <div className="mt32 pt16" style={{ borderTop: '1px solid var(--glass-border)', textAlign: 'center' }}>
          <div className="text-mute text-xs">© 2024 Top 1% OS. All rights reserved. Built for the elite.</div>
        </div>
      </footer>
    </div>
  );
    ...
    <>
      ...
      <div className="app">
        ...
        <div className="main" ref={mainRef}>
          ...
          {/* Top bar for auth pages nav hint */}
          {isAuthPage && (
            <div style={{ position: "absolute", top: 16, right: 20, display: "flex", gap: 8, zIndex: 10 }}>
              {[["Landing", 0], ["Sign Up", 1], ["Login", 2]].map(([label, id]) => (
                <button key={id} className={`btn ${page === id ? "btn-primary" : "btn-ghost"} `} style={{ padding: "6px 14px", fontSize: 12 }} onClick={() => setPage(id)}>{label}</button>
              ))}
            </div>
          )}
